...
abstract class AutonomoCompleto(
    ...
    val bigWobblePose = Pose2d(-10.2, -34.0, Math.toRadians(300.0)).invertIfNeededWithHeading()

    override fun setup() {
        super.setup()

        drive.poseEstimate = startPosition.startPose.invertIfNeededWithHeading()
    ...
    override fun sequence(teamMarkerPosition: TeamMarkerPosition) =
            drive.trajectorySequenceBuilder(startPosition.startPose.invertIfNeededWithHeading()).run {
                ...
                lineToSplineHeading(startPosition.startWobblePose.invertIfNeededWithHeading())

                waitSeconds(2.0)

                if(doDucks) {
                    // duck spinny boi
                    lineToLinearHeading(Pose2d(-66.5, -59.0, Math.toRadians(180.0)).invertIfNeededWithHeading())
                    UNSTABLE_addTemporalMarkerOffset(0.0) {
                ...
                if(cycles >= 1) {
                    ...
                    repeat(cycles) {
                        ...
                        splineToSplineHeading(
                            Pose2d(25.0, -63.3, Math.toRadians(0.0)).invertIfNeeded(),
                            Math.toRadians((-5.0).invertDegIfNeeded())
                        )
                        ...
                        // grab freight
                        splineTo(Vector2d(currentGrabCubeX, -63.2).invertIfNeeded(), 0.0)

                        // out of the warehouse
                        lineTo(Vector2d(18.0, -63.2).invertIfNeeded())
                        UNSTABLE_addTemporalMarkerOffset(0.0) {
                            + IntakeStopCmd()
                ...
                when(parkPosition) {
                    ...
                    WAREHOUSE -> {
                        ...
                        // park fully
                        lineTo(Vector2d(50.0, -64.0).invertIfNeeded())
                        // in case alliance wants to park too
                        strafeTo(Vector2d(50.0, -40.0).invertIfNeeded())
                    }
                    STORAGE_UNIT -> {
    ...
    fun Pose2d.invertIfNeededWithHeading() = if(alliance == Alliance.BLUE) {
